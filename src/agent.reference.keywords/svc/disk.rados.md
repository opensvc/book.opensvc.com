...
**Minimal configlet:**
	...
	[disk#1]
	type = rados
	name = pool1/cluster1/svc1

**Minimal setup command:**

	om test/svc/foo set \
		--kw="type=rados" \
		--kw="name=pool1/cluster1/svc1"
...
Setting `rwx` or `rox` will activate acquire shared locks.
...
**Example:**
	...
	config=from ./cfg/ceph key ceph.conf

**Description:**

This keyword specifies the datastore and key where the ceph.conf content is securely stored.

The content is automatically exposed on all nodes as a temporary file at /run/opensvc/<fqn>/key/<keyname>, allowing the file to be securely referenced by the rbd command using the -c <filename> flag.

If not set, the rbd command will fall back to /etc/ceph/ceph.conf.

Value format: `from <datastore path> key <key name>` (the path supports relative notation, e.g., ./cfg/ceph).
...
This keyword specifies the RBD group name for the image.

On provision, the group will be created if it doesn't exist, and the image will be added to the group if it's not already a member.

On status, a warning will be logged if the group doesn't exist or the image is not in the group.

On unprovision, the image will be removed from the group, and the group will be removed if it becomes empty.

Value format: a simple group name string.
...
**Example:**
	...
	keyring=from ./sec/ceph key eu1.keyring

**Description:**

This keyword specifies the datastore and key where the keyring content is securely stored.

The content is automatically exposed on all nodes as a temporary file at /run/opensvc/<fqn>/key/<keyname>, allowing the file to be securely referenced by the rbd command using the --keyring <filename> flag.

Value format: `from <datastore path> key <key name>` (the path supports relative notation, e.g., ./sec/ceph).
...
The image specifier in the storage system.
