...
The api url to use for filesystem management. Defaults to the files.base_url
value from the sgcp configuration file.
...
Defaults to the files.fs.exclusive value from the sgcp configuration file,
...
## Keyword `permission`
	...
	required:    false
	scopable:    true
	candidates:  read-only, read-write
	rbac:        Requires the root grant.

**Description:**

The access permission on the filesystem granted to the host. Defaults to the
files.fs.permission value from the sgcp configuration file, which itself
...
The protocol used to access the filesystem. Defaults to the files.fs.protocol
value from the sgcp configuration file, which itself defaults to nfs4.1.
...
The name of the secret object in the same namespace containing the api access
credentials. Defaults to the auth.default_secret value from the sgcp
configuration file.
...
Wait for `<duration>` before declaring the start action a failure.

If undefined, the object start_timeout will be used.
