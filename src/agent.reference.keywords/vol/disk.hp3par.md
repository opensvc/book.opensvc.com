...
Suffix of the array configuration section in cluster.conf (the part after # in array#<suffix>).


## Keyword `auto_takeover`
...
Allow failover when the arrays are split.
...
Trigger a sync before migrate if the rcg mode is Periodic.
...
Name of the Remote Copy Group (RCG) for volume replication.
...
Two times the rcg sync period.
**Description:**

The delay above which the status log of the resource reports a warning about last sync too old.

It should be set according to your application service level agreement.

The rcp sync period should be lower than `max_delay` minus the expected sync duration.
...
Replication mode. Supported values: sync, async (periodic).
...
Maximum duration to wait for start operations to complete.
...
Swap the natural replication direction after stopping the replication on migrate.
...
Maximum duration to wait for operations to complete.
